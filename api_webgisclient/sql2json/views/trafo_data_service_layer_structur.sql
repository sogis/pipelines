...
FROM 
    trafo_data_service_layer_basic_v basic
left join 
    simiproduct_properties_in_list properties_in_list 
    on 
    properties_in_list.product_list_id = basic.product_id 
left join 
    simiproduct_properties_in_facade properties_in_facade 
    on 
    properties_in_facade.facade_layer_id = basic.product_id
left join 
    simiproduct_single_actor single_actor_group 
    on 
    single_actor_group.id = properties_in_list.product_list_id
left join 
    simiproduct_single_actor single_actor_facade 
    on 
    single_actor_facade.id = properties_in_facade.facade_layer_id 
) 
...
left join 
    simiproduct_properties_in_facade properties_in_facade 
    on 
    properties_in_facade.facade_layer_id = first_sublevel.group_sublayer_single_actor_id
left join 
    simiproduct_single_actor single_actor 
    on 
    single_actor.id = properties_in_facade.facade_layer_id
;

GRANT SELECT ON TABLE trafo_data_service_layer_structur_v TO simi_write;
