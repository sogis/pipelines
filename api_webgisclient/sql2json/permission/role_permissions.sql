...
fl_perm as ( -- Aggregated permissions for a facadelayer. Derived from the contained singleactor permissions
	...
	inner join 
		simiiam_permission sp on pif.data_set_view_id = sp.data_set_view_id
	group by 
...
lg_perm as ( -- permissions for layer groups
	...
	inner join
		simiproduct_layer_group lg on pil.product_list_id = lg.id -- choose only productlists of type layergroup
	inner join 
...
perm_raw as ( -- lists all permissions for dataproducts in "human readable" form
	...
	from 
		dp_perm p
	inner join 
		simiproduct_data_product dp on p.dp_id = dp.id
),
...
from 
	perm_json p
inner join 
	simiiam_role r on p.role_id = r.id 
;
